...
| **Wikimedia Commons, YouTube, Flickr, Memorix, municipal archives, estate-agent and local-media sites** (via local Geo Media packs) | The Geo Media layer: internet-found photos and videos plotted at the camera position they were shot from | Per file: Wikimedia Commons images carry their own licence (mostly CC BY-SA); YouTube clips stay their uploader's and are only embedded (youtube-nocookie), never downloaded | The author, capture date, licence, and a link to the original are shown in the Geo Media detail panel for every item |
...
- **Geo Media.** The layer ships no third-party media of its own. Each pack is a small local JSON file under `public/geo-media/` listing a media item's camera position, viewing direction, and the URL of an image or clip hosted by its original platform. Images are loaded directly by the browser from the publisher's own host; a host that sends no `Access-Control-Allow-Origin` cannot texture the 3D plane, which then stays a flat tinted pane while the panel still shows the picture. Items that are only a viewpoint (Street View panoramas: pano id, position and bearing, no image) draw a pin and a wedge and nothing else. video plays in a youtube-nocookie iframe that exists only while its item is selected. Nothing is cached, rehosted, or redistributed. Poses in a pack are best-effort: EXIF gives a position, almost never a bearing, so every item carries its own `positionConfidence` / `headingConfidence` and can be hand-corrected in the app. A wedge on the map is where someone stood, not evidence of what is there now.
